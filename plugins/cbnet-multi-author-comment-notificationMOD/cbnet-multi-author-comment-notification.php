	...
	function cbnetmcnOptionsPg() {
		...
		define('DIEONDBERROR', true);
			...
			foreach ($result as $row)  {
				...
				if (is_multisite()) {
					$data->wp_user_level = intval(get_user_meta($row->ID, $wpdb->base_prefix . get_current_blog_id() . '_user_level',true));
				} else {
				...
				if (is_multisite()) {
					$level    = @key(get_user_meta($row->ID, $wpdb->base_prefix . get_current_blog_id() . '_capabilities',true));
				} else {
			...
			<div class="wrap">
			 ...
			 <pre>
			 <?php //print_r(get_users(get_current_blog_id()));?>
			 </pre>
			 <form method="post">
			 <p>
