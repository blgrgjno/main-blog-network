<?php
//				$hash = md5( 'Cookie set here! ' . rand(), time()-345999 );
//				$hash = md5( 'Cookie set here! ' . rand() );
//				setcookie( 'comment_ratings', $hash, time() + YEAR_IN_SECONDS );
function cr_log( $string ) {
	$old = file_get_contents( '/home/ryan/Desktop/www/dss/wp_mail.txt' );
	file_put_contents( '/home/ryan/Desktop/www/dss/wp_mail.txt', $old . $string . "\n" );
	return;
}
...
class Comment_Ratings {
	...
	public function ajax_vote() {
		...
		if ( ! isset( $_REQUEST['cookie'] ) && ! isset( $_REQUEST['comment_id'] ) && ! isset( $_REQUEST['post_id'] ) && ! isset( $_REQUEST['action'] ) )
			return;
