...
class instagrate_pro {
	...
	function custom_content_header() {
		...
		$link = ' <a target="_blank" style="color: #21759B" href="http://www.instagrate.co.uk/help#templates">'. $link_text .'</a>';
			...
			if (!isset($options['token']) || $options['token'] == '') {
				...
				<table class="form-table igp-admin"> 
					...
					<tr valign="top" class="instagram_user">
		                ...
		                <td>
							<input type="text" id="igp-users-name-input" name="_instagrate_pro_settings[instagram_user]" value="<?php echo $this->default_val($options, 'instagram_user', ''); ?>" /><br />
					...
					<tr valign="top" class="instagram_location">
		                ...
		                <td>
							<input type="text" id="igp-location-name-input" name="_instagrate_pro_settings[instagram_location]" value="<?php echo $this->default_val($options, 'instagram_location', ''); ?>" /><br />
	       ...
	       _e( 'You will need to connect an Instagram account from the Instagram settings', $this->plugin_l10n );	        
			...
			<div id="igp-image-manager">
				...
				<div id="igp-bulk-wrap" class="igp-bulk" <?php echo $show_bulk; ?>>
					...
					<input id="igp-set-bulk-status" type="button" class="button" value="Bulk Set Status">
				</div>
			</div>
	...
	function meta_box_map() {
        ...
        <table class="form-table igp-admin"> 
			...
			<tr valign="top">
                ...
                <td>
					<input type="text" name="_instagrate_pro_settings[map_css]" value="<?php echo $this->default_val($options, 'map_css', ''); ?>" /><br />
                </td>
            </tr>
			<tr valign="top">
	...
	function get_locations($account_id, $location, $lat, $lng) {
        $account_settings = get_post_meta($account_id, '_instagrate_pro_settings', true );
        $access = $account_settings['token'];
		$options = get_post_meta( $account_id, '_instagrate_pro_settings', true ); 
		$url = 'locations/search/';
		$new_locations = array();
		$distance = $this->default_val($this->settings, 'igpsettings_general_location-distance', '1000');
		if ($lat == '' || $lng == '') {
			$new_locations[0] = '— '. __('Enter Location', $this->plugin_l10n). ' —';
			return $new_locations;
		}
		$new_locations[0] = '— '. __('No Locations Found', $this->plugin_l10n). ' —';
		$params = array('lat' => $lat, 'lng' =>  $lng, 'distance' => $distance);
		$data = $this->do_http_request( $access, $url, $params);
		if (!$data) return;
		if ($data->meta->code == 200) {
			$locations = $data->data;
	...
	function igp_get_locations_callback() {
        ...
        $response['error'] = false;
        $response['message'] = '';
        $locations = $this->get_locations($_POST['post_id'], $_POST['location'], $_POST['lat'], $_POST['lng']);
...
$this->make_debug('Single '. $settings->post_type .' but not the single '. $settings->post_type .'('. $settings->posting_same_post .')' );
				...
				case 'page':
					if ($settings->posting_multiple != 'single') {
						if ( !is_front_page() && !is_category( $settings->post_term )  && !is_tax( $settings->post_taxonomy, $settings->post_term ) ) {
$this->make_debug('Not single '. $settings->post_type .' but not front page or category page('. $settings->post_term .')' );
							return $this->write_debug($account_id);
						}
					} else {
...
$this->make_debug('Image not pending');
					...
					$instagram_date = date('Y-m-d H:i:s', $image->image_timestamp);
