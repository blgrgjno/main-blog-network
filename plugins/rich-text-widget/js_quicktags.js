...
function edShowButton(which, button, i) {
	...
	switch (button.id) {
		case 'ed_img':
			html +='<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertImage(\'' + which + '\');" value="' + button.display + '" />';
			break;
		case 'ed_link':
			html = html + '<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertLink(\'' + which + '\', ' + i + ');" value="' + button.display + '" />';
			break;
		case 'ed_ext_link':
			html = html + '<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertExtLink(\'' + which + '\', ' + i + ');" value="' + button.display + '" />';
			break;
		case 'ed_footnote':
			html = html + '<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertFootnote(\'' + which + '\');" value="' + button.display + '" />';
			break;
		case 'ed_via':
			html = html + '<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertVia(\'' + which + '\');" value="' + button.display + '" />';
			break;
		default:
			html = html + '<input type="button" id="' + button.id + '_' + which + '" ' + accesskey + ' class="ed_button" onclick="edInsertTag(\'' + which + '\', ' + i + ');" value="' + button.display + '"  />';
			break;
	}
	return html;
}
...
function edShowLinks() {
	...
	for (i = 0; i < edLinks.length; i++) {
		tempStr = tempStr + '<option value="' + i + '">' + edLinks[i].display + '</option>';
	}
...
function edToolbar(which) {
	var html = '';
	html = html + '<div id="ed_toolbar_' + which + '"><span>';
	for (i = 0; i < extendedStart; i++) {
		html += edShowButton(which, edButtons[i], i);
	}
	html = html + '<input type="button" id="ed_spell" class="ed_button" onclick="edSpell(\'' + which + '\');" title="' + quicktagsL10n.dictionaryLookup + '" value="' + quicktagsL10n.lookup + '"  />';
	html = html + '<input type="button" id="ed_close" class="ed_button" onclick="edCloseAllTags(\'' + which + '\');" title="' + quicktagsL10n.closeAllOpenTags + '" value="' + quicktagsL10n.closeTags + '" />';

	html = html + '</span>';
//	edShowLinks();
	html = html + '</div>';
	/*alert(jQuery('#'+which+' #quicktags').attr('id'));
	alert('#'+which+' #quicktags');*/
	var quicktags = jQuery('#'+which).parents('.widget').find('#quicktags');
	quicktags.html(html);
