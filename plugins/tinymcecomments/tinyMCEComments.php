...
function mcecomment_getInitJS($debugMode=0) {
   global $mce_locale;
	...
	$initArray = array (
		'mode' => 'exact',
		'elements' => 'comment,comment_where,comment_how',
		'theme' => 'advanced',
		...
		'valid_elements' => "h1,h2,h3,h4,h5,a[name|href|title],hr[class|width|size|noshade],font[face|size|color|style],span[class|align|style],blockquote[cite]".$le,
		'language' => $mce_locale,
   );
...
function mcecomment_admin_page_init() {
   ...
   wp_enqueue_script('tiny_mce', get_option('siteurl') . '/wp-includes/js/tinymce/tiny_mce.js', false, '20081129');
	...
	wp_enqueue_script('tiny_mce_lang', get_option('siteurl') . '/wp-includes/js/tinymce/themes/advanced/langs/' . $mce_locale . '.js', false, '20081129');
	//E:\xampplite\!sites\hod\htdocs\wp-includes\js\tinymce\themes\advanced\langs
	mcecomment_getInitJS();
}
