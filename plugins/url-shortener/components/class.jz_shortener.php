<?php
/*
 * class.jz_shortener - URL Shortener Plugin Component
...
/*
    Class Description
        ...
        For usage, please refer to http://wiki.fusedthought.com/
...
if ( !class_exists('jzsc_simple') ) :
	include( dirname(__FILE__) . '/jz_shortener/jzsc.simple.php' );
endif;

if ( !class_exists('jzsc_complex') ) :
	include( dirname(__FILE__) . '/jz_shortener/jzsc.complex.php' );
endif;

if ( !class_exists('jzsc_googl') ) :
	include( dirname(__FILE__) . '/jz_shortener/jzsc.googl.php' );
endif; 


if ( !class_exists('jzsc_yourls') ) :
	include( dirname(__FILE__) . '/jz_shortener/jzsc.yourls.php' );
endif; 
*/
 ...
 */
        ...
        public function __construct($status = 'dev') {
            $this->jz_shortener($status);
        }
       
        //backward compatibility
        public function jz_shortener($status = 'dev'){
            $this->status = $status;
			$this->init_modules();
 ...
 */
		...
		private function init_modules(){
			$dirpath = dirname(__FILE__) . '/jz_shortener';
		...
		public function generate($url){
			...
			if ( !empty($this->shortener) ){
				...
				}else{ $data = ''; //'No URL specified';
				}

			}else{ $data = ''; //'Shortener not initialized';
			}
