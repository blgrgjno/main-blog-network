...
?>
				<div class="nopassword"><?php _e('Dette innspillet er passordbeskyttet. Skriv inn passordet ditt for &aring; lese kommentarer.', 'thematic') ?></div>
			</div><!-- .comments -->
<?php
		return;
	endif;
...
<?php 
// TODO: move news category 9 to config
if ( 'open' == $post->comment_status && get_theme_option('enable_statements') && !in_category('nyheter')) : ?>
				<div id="respond">
    				<h3 class="buttonRoundSlim"><span><?php comment_form_title( __('Kommenter innspillet', 'thematic'), __('Skriv et svar til %s', 'thematic') ); ?></span></h3>
					<?php 
						if (!is_page()) {
							// Report post
							wprp(true);
...
<?php endif /* if ( $user_ID ) */ ?>
                  ...
                  <?php do_action('comment_form', $post->ID); ?>
							...
							<input type="hidden" name="redirect_to" value="<?php echo esc_url( htmlentities(strip_tags($_SERVER['REQUEST_URI']), ENT_QUOTES ) ); ?>" />
