...
function send_notifications($postID) {
	...
	if (get_theme_option('version') == 'minimal') {
		$template_name = 'email_notification_minimal.html';
	}
	else if ($authorType == "virskomhet") {
		$template_name = 'email_notification_virksomhet.html';
	}
	else {
		$template_name = 'email_notification_privatperson.html';
	}
	...
	// subject
	$subject = 'Ditt '.strtolower(get_theme_option('entity_singular')).', '.get_the_title().', er mottatt';
	...
	// Additional headers
	$headers .= 'From: '.get_theme_option('sender_name').' <'.get_theme_option('sender_email').'>' . "\r\n";
