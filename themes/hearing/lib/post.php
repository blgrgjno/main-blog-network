...
function get_comments_bubble() {
	...
	$postcomments = '<span class="comment_bubble"><a href="' . get_permalink( $post->ID ) . '#comments" title="' . __('Les kommentarer til ', 'thematic') . the_title_attribute('echo=0') . '">'.$postcommentnumber . __('', 'thematic') . '</a></span>';
...
function my_postheader() {
    ...
    if (is_single() || is_page()) {
		...
		$statement_meta = get_statement_meta($post->ID);
        $posttitle = '<a href="';
		if ($post->post_type == "post" && !in_category('nyheter')) {
    ...
    if(!is_single() && $post->post_type == "post" && !is_404()) {
		if (!is_author()) {
			echo '<div class="author-meta"><span class="author_name author_"><span class="a_alt">';
			the_author_link();
			echo '</span></span></div>';
		}
...
function my_postfooter() {
    ...
    if (is_single() || is_page()) {
      if (in_category('nyheter')) {
        $postfooter = '<a href="/category/nyheter"><h3 class="buttonRoundSlim"><span>Alle nyheter</span></h3></a>';
      } else {
        $postfooter = '';
      }
    } elseif (is_404()) {    
		...
		if ($post->post_type == "post") {
          $read_post = get_theme_option('read_post');
          $read_link = $statement_meta->statement_url;
          if (in_category('nyheter')) {
            $read_post = get_theme_option('read_news');
            $read_link = get_permalink();
          }
			$readfull  = '<span class="meta_readmore"><a href="'.$read_link.'">'.str_replace("{author_name}", $authordata->display_name, $read_post).'</a></span>';
		}
		else {
