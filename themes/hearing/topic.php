<?php
		...
		switch ($show) {
			...
			case $nhop['slug_write']:
				if ($nhop['enable_statements']) {
					$title_before = $nhop['write_entry_page_title'] . " | ";
				}
				break;
			default:
				$title_before = "";
	...
	function nhop_page_header() {
		...
		if ($show == $nhop['slug_statement']) {
			$pretitle = "<div class='page_header_pretitle'>".get_the_title()."</div>";
			$post = get_post($statement_id, OBJECT);
			setup_postdata($post);
			$title = get_the_title();
		}
		else if ($show == $nhop['slug_write']) {
			//$pretitle = "<div class='page_header_pretitle'>".get_the_title()."</div>";
			$title = $nhop['write_entry_page_title'];
		}
?>
		<div id="c_page_header" class="page_header_topic">
			<div class="page_header">
				...
				?>
<?php
		if ($nhop['version'] == 'normal' && $show != $nhop['slug_write']) {
			topic_select();
		}
...
<?php
		if ($nhop['version'] == 'normal' && ($show != $nhop['slug_write'] || !$nhop['enable_statements'])) {
			$permalink = get_permalink($post->ID);
?>
			<div class="topic-tabs-container">
				<ul class="topic-tabs">
					...
					<?php if($meta_values['topic_summary_sit'][0]) { ?>
						<li class="<?php echo $class_summary; ?>"><a href="<?php echo esc_url( $permalink ); ?>"><?php echo $nhop['tab_summary']; ?></a></li>
					<?php } ?>
					<?php if($meta_values['topic_main_sit'][0]) { ?>
						<li class="<?php echo $class_full; ?>"><a href="<?php echo esc_url( $permalink ); ?><?php echo $nhop['slug_full']; ?>/"><?php echo $nhop['tab_full']; ?></a></li>
					<?php } ?>
					<?php if($meta_values['topic_examples'][0]) { ?>
						<li class="<?php echo $class_examples; ?>"><a href="<?php echo esc_url( $permalink ); ?><?php echo $nhop['slug_examples']; ?>/"><?php echo $nhop['tab_examples']; ?></a></li>
					<?php } ?>
					<li class="<?php echo $class_statements; ?>"><a href="<?php echo esc_url( $permalink ); ?><?php echo $nhop['slug_statements']; ?>/"><?php echo $nhop['tab_statements']; ?></a></li>
				</ul>
			</div>
<?php
		}
...
<?php
	if ($nhop['enable_statements']) {
?>
					<p class="buttons"><a href="<?php echo esc_url( $topic_url.$nhop['slug_write']."/" ); ?>" class="buttonRound"><span><?php echo $nhop['send_answer']; ?></span></a></p>
<?php
	}
?>
				</div><!-- /entry-content -->
...
?>
			<a href="<?php echo $topic_url.$nhop['slug_write']."/"; ?>" class="buttonRound"><span><?php echo $nhop['send_answer']; ?></span></a>
<?php
		}
?>
		</div><!-- /write-entry -->
...
?>
			<?php if ($meta_values['topic_main_sit_header'][0]) { ?>
				...
				<div class="<?php thematic_post_class() ?>">
					<h2 class="entry-title"><?php echo $meta_values['topic_main_sit_header'][0]; ?> <?php echo get_social_bookmarks(); ?></h2>
					<div class="entry-content">
...
?>
		<div id="post-<?php the_ID(); ?>" class="<?php thematic_post_class() ?>">
			<h2 class="entry-title"><?php echo $nhop['tab_examples']; ?> <?php echo get_social_bookmarks(); ?></h2>
			<div class="entry-content">
<?php
...
<?php
		...
		echo get_social_bookmarks();
		echo get_theme_option('entity_plural')." <span class='number'>(".$wp_query->found_posts.")</span>";
		echo "</h2>";
...
?>
		<div class="postSorter">
			<a href="<?php echo esc_url( $sort_base_url ); ?>" <?php if ($_GET['sortering'] != "kommentarer") echo "class='active'"; ?>>Siste</a> |
			<a href="<?php echo esc_url( $sort_base_url ); ?>?sortering=kommentarer" <?php if ($_GET['sortering'] == "kommentarer") echo "class='active'"; ?>>Mest kommentert</a>
		</div>
<?php
...
<?php
		...
		echo get_social_bookmarks(false, true);
...
?>
			<div class="<?php thematic_post_class() ?>">
				<h2 class="entry-title"><?php echo get_social_bookmarks(); ?> <?php echo $meta_values['topic_summary_sit_header'][0]; ?></h2>
				<div class="entry-content">
			...
			<?php if ($meta_values['topic_summary_goal_header'][0]) { ?>
				<div class="<?php thematic_post_class() ?>">
					...
					<?php if ($meta_values['topic_main_goal'][0]) { ?>
						<p class="meta_readmore"><a href='<?php echo esc_url( $permalink.$nhop['slug_full'] ); ?>/#maal'>Mer om <?php echo mb_strtolower($meta_values['topic_summary_goal_header'][0], 'UTF-8'); ?></a></p>
					<?php } ?>
				</div>
			<?php } ?>
...
?>
			...
			<p class=""><a href='<?php echo esc_url( $permalink.$nhop['slug_full']."/" ); ?>'>Mer om <?php echo mb_strtolower(get_the_title(), 'UTF-8'); ?></a></p>
<?php
		}
?>
<?php
