...
$theme_options_defaults = array(
	...
	'exit_url' => 'http://www.regjeringen.no/nb/dep/hod.html',
	'Avansert' => '====',
	'mainmenu_slug' => 'menu-hovedmeny',
	'footermenu_slug' => 'bunnmeny',
...
function childtheme_add_admin() {
	add_submenu_page('themes.php', 'NHOP-innstillinger', 'NHOP-innstillinger', 'edit_themes', basename(__FILE__), 'childtheme_admin');
}
	...
	if ($_POST['options-submit']) {
		...
		foreach ($theme_options_defaults as $key => $value) {
			...
			if (isset($_POST[$option_name])) {
				$option_var = wp_kses( $_POST[$option_name], '', '' ); // Arbitrary use of sanitisation due to needing to fix this in a hurry (during security audit) and not being sure what the code actually does
				update_option($option_name, $option_var );
			}
		}
		...
		?>
			<div class="updated"><p><strong>Innstillinger lagret.</strong></p></div>
		<?php
