...
<?php
	...
	function widget($args, $instance) {
		// Don't display on list pages:
		global $wp_query;
		if ($wp_query->query_vars['show'] == "svar" || is_home() || is_author()) return;
...
<?php
	...
	function print_latest($args, $instance) {
		...
		global $post;
		$querystr = array('showposts' => $number, 'nopaging' => 0, 'post_status' => 'publish', 'caller_get_posts' => 1);
		...
		$title = apply_filters('widget_title', empty($instance['title_latest']) ? "Siste høringssvar" : $instance['title_latest'], $instance, $this->id_base);
		if ($title) echo "<h4>" . $title . "</h4>";
?>
		<ul class="nhop_latest_posts">
<?php
...
?>
		<li>
			<div class="author author_<?php echo $user_type; ?>"><?php the_author_posts_link() ?></div>
			<div class="post"><a href="<?php echo $statement_meta->statement_url ?>" title="<?php echo $post_title; ?>"><?php echo truncate($post_title, 80, $ending = ' (...)', false); ?></a> <?php echo get_comments_bubble(); ?></div>
