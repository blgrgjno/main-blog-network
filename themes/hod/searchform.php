...
if (is_search()) {
	$search_form .= '<input id="s" name="s" type="text" value="' . esc_attr( wp_specialchars(stripslashes($_GET['s']), true) ) .'" size="32" tabindex="1" />';
} else {
	$value = $SEARCH_TEXT; //__('Search');
	$value = apply_filters('search_field_value',$value);
	$search_form .= '<input id="s" name="s" type="text" value="' . esc_attr( $value ) . '" onfocus="if (this.value == \'' . $value . '\') {this.value = \'\';}" onblur="if (this.value == \'\') {this.value = \'' . esc_attr( $value ) . '\';}" size="32" tabindex="1" />';
}
$search_form .= '</form>';
$search_form .= '</div>';
